...
# 终端运行指令
# python train.py --dataset CVACT --data_dir ./datasets/CVACT/ --n_des 8 --TR_heads 4 --TR_layers 2 --layout_sim strong --sem_aug strong --pt --cf
...
if __name__ == "__main__":
    ...
    save_name = f"{ts}_{opt.dataset}_{opt.cf}_{opt.pt}_{opt.save_suffix}"
    ...
    for epoch in range(start_epoch, number_of_epoch):
        ...
        with torch.no_grad():
            for batch in tqdm(validateloader, disable = opt.verbose):
                sat = batch['satellite'].to(device)
                grd = batch['ground'].to(device)
                sat_global, grd_global = model(sat, grd, False)
