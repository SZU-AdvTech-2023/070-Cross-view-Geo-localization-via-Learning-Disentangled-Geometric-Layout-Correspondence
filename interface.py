...
if __name__ == "__main__":
    ...
    if opt.dataset == 'CVACT':
        ...
        dataset = ACTDataset(data_dir=data_path, layout_simulation='none', sematic_aug='none', is_polar=opt.pt,
                             mode='val')
        validateloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=8)
    if opt.dataset == 'CVUSA':
        data_path = opt.data_dir
        dataset = USADataset(data_dir=data_path, layout_simulation='none', sematic_aug='none', mode='val',
                             is_polar=opt.pt)
